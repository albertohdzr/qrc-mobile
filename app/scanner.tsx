...
export default function ScannerScreen() {
  ...
  const { setWalletInfo, clearWallet } = useCartStore()
  ...
  useEffect(() => {
    // Limpiar wallet al entrar al scanner
    clearWallet()
    return () => {
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      // Guardar la info de la wallet y navegar al checkout
      setWalletInfo({
        code5,
...
const styles = StyleSheet.create({
  ...
  scanArea: {
    ...
    position: 'relative',
    // Clear the overlay in this area
    overflow: 'hidden',
  },
  corner: {
    position: 'absolute',
